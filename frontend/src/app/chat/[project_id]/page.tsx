...
export default function ChatPage({ params }: PageProps) {
  ...
  // Scroll timeline to bottom on new messages
  ...
  // Load project meta details to render top header labels
  useEffect(() => {
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            {messages.map((msg, index) => (
              <div key={index} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div className="text-xs text-zinc-600 mb-1 font-mono uppercase tracking-wider px-2">
                  {msg.role}
                </div>
                ...
                <div className={`max-w-3xl rounded-2xl px-5 py-4 text-sm leading-relaxed border ${
                  ...
                  <div className="whitespace-pre-wrap font-sans text-zinc-200">{msg.content}</div>
                </div>
                ...
                {msg.role === "assistant" && msg.sources && msg.sources.length > 0 && (
                  <div className="max-w-3xl w-full mt-3 space-y-2 px-1">
                    <span className="text-xs font-mono text-zinc-500">Retrieved Context Sources:</span>
